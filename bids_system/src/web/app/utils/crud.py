...
class CrudAPIRouter(_CrudAPIRouter):
    def __init__(
        ...
        get_all_route = get_all_route or RouteDict(dependencies=[Depends(get_current_user())],
                                                   responses={**missing_token_or_inactive_user_response})
        get_one_route = get_one_route or RouteDict(dependencies=[Depends(get_current_user())],
                                                   responses={**missing_token_or_inactive_user_response,
                                                              **not_found_response})
        create_route = create_route or RouteDict(dependencies=[Depends(get_current_user())],
                                                 responses={**missing_token_or_inactive_user_response,
                                                            })
        update_route = update_route or RouteDict(dependencies=[Depends(get_current_user(superuser=True))],
                                                 responses={**missing_token_or_inactive_user_response,
                                                            })
        delete_all_route = delete_all_route or RouteDict(dependencies=[Depends(get_current_user(superuser=True))],
                                                         responses={**auth_responses})
        delete_one_route = delete_one_route or RouteDict(dependencies=[Depends(get_current_user(superuser=True))],
                                                         responses={**auth_responses,
                                                                    **not_found_response})
        super().__init__(
            schema=schema,
